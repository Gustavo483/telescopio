...
@section('conteudo')
    ...
    <div class="flexPAluno mt-3 ">
        <h4 class="h4Pets">
            Painal do Administrador > Unidades > Conteudos > Editar questão ME
        </h4>
        <a class="btnVoltar" href="{{route('vizualizar.TodasAtividades',['dadosconteudo'=>$Questao->fk_conteudo])}}">
            Voltar
        </a>
    </div>

    <h3 class=" mt-5 h4Pets text-center mb-5">
        Editar questão de múltipla escolha para o conteúdo {{$conteudo->st_nome_conteudo}}
    </h3>

    <form method="post" action="{{ route('update.QuestaoME', ['IDQuestao' =>$Questao->id])}}">
        @csrf
        @method('PUT')
