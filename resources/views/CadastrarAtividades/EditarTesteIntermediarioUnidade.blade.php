...
@section('conteudo')
    <div class="flexPAluno mt-3 ">
        <h4 class="h4Pets">
            Painal do Administrador > Unidades > Conteudos > Editar teste Final intermediario
        </h4>
    ...
    <div class="flexRegisterAluno2 mb-5">
        <div class="FormCadastro">
            <form method="post" action="{{route('updateTesteIntermediarioUnidade.conteudo', ['dadosconteudo'=>$dadosconteudo])}}">
                @csrf
                <div class="hsd2">
                    <h4>teste intermediario da unidade: {{$dadosconteudo->unidades->st_nome_unidade}}</h4>
                </div>
                <div class="p-2">
                    ...
                    @foreach($todosConteudo as $conteudo)
                        @if(!in_array($conteudo->id,$IdPreenchidos))
                            <div>
