...
@section('conteudo')
    <div class="flexPAluno mt-3 ">
        <h4 class="h4Pets">
            Painal do Administrador > Unidades > Conteudos > vizualizar teste final da unidade
        </h4>
    ...
    <h3 class=" mt-5 h4Pets text-center">
        Relação de conteudos para o teste final da unidade {{$dadosconteudo->unidades->st_nome_unidade}}
    </h3>

    <a class="btnVoltar5 my-3 text-center" href="{{route('edit.ConteudoTesteFinal',['dadosconteudo'=>$dados[0]->fk_conteudo_pertencente])}}">
        Editar teste
    </a>
    ...
    <table id="myTable" class="mb-5 mt-5">
        ...
        @foreach($dados as $dado)
            <tr class="border12">
                ...
                <td class="border12">
                    <form id="form_{{$dado->id}}" method="post" action="{{route('delete.ConteudoTesteFinal',['IDexclusao'=>$dado->id, 'dadosconteudo'=>$dado->fk_conteudo_pertencente])}}">
                        @method('DELETE')
                        @csrf
